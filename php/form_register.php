...
if(isset($_POST['save']))
{
	...
	if(isset($_GET['data_id']))
	{
		...
		$sql="update users set name='$name',email='$email',password='$password',address='$address',concact='$contact',city='$city',gender='$gender',hobby='$hobby_str',image='$image' where id=".$user_id;
	}else
	{
		move_uploaded_file($_FILES['image']['tmp_name'],'upload/'.$image);

		$sql="insert into users(name,email,password,address,concact,city,gender,hobby,image) values ('$name','$email','$password','$address','$contact','$city','$gender','$hobby_str','$image');";
...
<body>
	<form method="post" enctype="multipart/form-data">
		<table border="1">
			...
			<tr>
				<td>contact</td>
				<td><input type="text" name="contact" value="<?php echo @$row['concact']; ?>"></td>
			</tr>
			<tr>
				<td>city</td>
				<td>
